...
io.on("connection", function (socket){
    ...
    socket.on("send new Todo from client", function(newToDo){
        console.log("received new todo list from the client");
        var newToDoFromClient = new ToDo({"description":description, "tags":tags});
        newToDoFromClient.save(function(err, result){
            if(err !== null){
                console.log(err);
                throw err;
            } else {
